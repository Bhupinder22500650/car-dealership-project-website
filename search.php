...
if ($brand !== '')        { $conditions[] = "company_name LIKE ?"; $params[] = "%{$brand}%"; $types .= 's'; }
if ($model !== '')        { $conditions[] = "car_model LIKE ?";    $params[] = "%{$model}%"; $types .= 's'; }
if ($year  !== '')        { $conditions[] = "car_year = ?";         $params[] = (int)$year;  $types .= 'i'; }
if ($price !== '')        { $conditions[] = "price <= ?";           $params[] = (float)$price; $types .= 'd'; }
if ($transmission !== '') { $conditions[] = "transmission = ?";     $params[] = $transmission; $types .= 's'; }
if ($fuel_type !== '')    { $conditions[] = "fuel_type = ?";        $params[] = $fuel_type;  $types .= 's'; }
if ($mileage !== '')      { $conditions[] = "mileage <= ?";         $params[] = (int)$mileage; $types .= 'i'; }

$order = match($sort) {
    'price_desc' => 'ORDER BY price DESC',
    'price_asc'  => 'ORDER BY price ASC',
    default      => 'ORDER BY car_id DESC'
};

$sql = "SELECT * FROM cars" . (count($conditions) ? " WHERE " . implode(' AND ', $conditions) : '') . " $order";
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16 md:gap-y-20">
            <?php foreach ($cars as $car):
                $img_src = !empty($car['image_url']) ? $car['image_url'] :
                           (!empty($car['image_path']) && file_exists(__DIR__.'/'.$car['image_path']) ? $car['image_path'] : 'https://lh3.googleusercontent.com/aida-public/AB6AXuBG9RNwfNEOsIQy5IkhkNJ9ZYR0VPOiuwI5HeztB-zGiVivol9KN1QdLMGuWqYA2aRY9khPVihZCRjyrtKOPwbQralx9wUak1u0hS4Mag_5LfCzAe40V9ynKee7yJ6ZzhRFwphHCQFi3NBhp_SqepqEvDFEy6xkAtCkyKx5tgw4q9ADsaQFSpREh8Ouz_W0fkGWETRqX5Z2Gg7MCD3rwrDRy3IXNHBAuqvAPvr8n6EODxlMoQ_KOXVwvNdzo_q5MPydzha0KZXjeQY');
