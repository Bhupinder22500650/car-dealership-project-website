...
if ($car_id <= 0) { header('Location: index.php'); exit; }

$stmt = $conn->prepare("SELECT c.*, u.username as seller_name, u.profile_photo as seller_photo FROM cars c LEFT JOIN users u ON c.seller_id = u.user_id WHERE c.car_id = ?");
...
$feedback_stmt = $conn->prepare("SELECT f.comment, u.First_name, u.Last_name, f.created_at FROM feedback f JOIN users u ON f.user_id = u.user_id WHERE f.car_id = ? ORDER BY f.created_at DESC LIMIT 3");
$feedback_stmt->bind_param('i', $car_id);
$feedback_stmt->execute();
$reviews = $feedback_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$feedback_stmt->close();
...
$img_src = !empty($car['image_url']) ? $car['image_url'] :
           (!empty($car['image_path']) && file_exists(__DIR__.'/'.$car['image_path']) ? $car['image_path'] :
