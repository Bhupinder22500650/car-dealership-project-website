...
// Fetch some recent listings for the featured section
$featured_cars = [];
if (isset($conn)) {
$result = $conn->query("SELECT car_id, company_name, car_model, car_year, price, fuel_type, image_url FROM cars ORDER BY car_id DESC LIMIT 4");
    if ($result) {
    ...
    <section class="bg-[#faf9f9] py-24 md:py-32 overflow-hidden">
        ...
        <div class="flex overflow-x-auto gap-8 md:gap-10 px-8 md:px-12 pb-12 snap-x snap-mandatory no-scrollbar">
            <?php foreach ($featured_cars as $car): 
                $car_img = !empty($car['image_url']) && file_exists(__DIR__ . '/' . $car['image_url']) ? $car['image_url'] : 'https://lh3.googleusercontent.com/aida-public/AB6AXuBI1eRAFBqSNU4dAs1OLZ3TPIE8-NtbZg7cvmLpa2P5VrIsGXKXSDGQx2kPcxqUODTpuU6UijKYCRnU93MutrkPy1B7mAfKb12FaloUcd9ewBc3zPb_zNvjaWu7bZFuACsCtov2S7yXqNZ_k5M3jR43dDKV_Y6mCYRFua0Cah5bmNMS2FtWOo8MD-7GkyeQ1wd8Fem99phi1x-D-QXYEXTequWTOkLUsVp2HysGh_-sMUTADI4GsB8nFohLi-Ikw7DPHWzppBtAaZg';
            ?>
